...
import cyborgMarkdown from '../cyborg.md?raw';
import { siteUrl } from '@/lib/repo-guide-pages';

const href = '/games/brotato/cyborg';
const h1 = 'Brotato Cyborg Guide: Minigun, Engineering and Build Strategy';
const description = "Learn how Cyborg's mid-wave Ranged Damage-to-Engineering conversion works, how Minigun and Wrench fit together, and what RUNBACK verified through Wave 8.";
const publishedAt = '2026-09-20T00:00:00.000Z';
const sources = [
  'https://brotato.wiki.spellsandguns.com/Cyborg',
  'https://brotato.wiki.spellsandguns.com/Engineering',
];
const images = {
  'CYB-01.png': { width: 2038, height: 1143, alt: 'Brotato Cyborg character traits showing the Ranged Damage to Engineering conversion' },
  'CYB-02.png': { width: 2266, height: 1278, alt: 'Cyborg Wave 4 shop with Minigun and three Wrenches' },
  'CYB-03-1.png': { width: 1920, height: 1080, alt: 'Cyborg Wave 5 before midpoint conversion with 12 Ranged Damage and 0 Engineering' },
  'CYB-03-2.png': { width: 1920, height: 1080, alt: 'Cyborg Wave 5 after midpoint conversion with 0 Ranged Damage and 24 Engineering' },
  'CYB-04.png': { width: 2277, height: 1273, alt: 'Cyborg Wave 8 shop showing 15 Ranged Damage and defensive stats' },
} as const;
...
function MarkdownBody() {
  const lines = cyborgMarkdown.replace(/\r\n/g, '\n').split('\n');
  ...
  while (index < lines.length) {
    ...
    if (image) {
      ...
      if (asset) blocks.push(<figure key={index} className="mt-6">{/* oxlint-disable-next-line next/no-img-element */}<img src={'/images/brotato/cyborg/' + image[1]} alt={asset.alt} width={asset.width} height={asset.height} loading="lazy" decoding="async" className="h-auto w-full rounded-xl" />{caption && <figcaption className="mt-3 text-sm leading-6 text-[#aeb7bc]">{caption}</figcaption>}</figure>);
      index += caption ? 3 : 1;
      continue;
    }
...
export default function CyborgGuide() {
  const schema = {
    ...
    '@graph': [
      {
        ...
        citation: sources,
        image: Object.keys(images).map(name => siteUrl + '/images/brotato/cyborg/' + name),
      },
      {
